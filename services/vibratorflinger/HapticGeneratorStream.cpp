...
status_t HapticGeneratorStream::read(VibratorQueues& queues, size_t bufferSize, int8_t* buffer,
                                     size_t* bytesRead) {
    *bytesRead = 0;
    if (!mIsActive) {
    ...
    while (systemTime(SYSTEM_TIME_MONOTONIC) < deadline && totalBytesRead < bufferSize) {
        ...
        size_t remainingLength = bufferSize - totalBytesRead;
        int8_t* currentBufferPtr = buffer + totalBytesRead;

        HapticGeneratorCommand command;
        command.set<HapticGeneratorCommand::Tag::burstBytes>(static_cast<int32_t>(remainingLength));
        HapticGeneratorReply reply;
        status_t status = sendCommandAndReceiveReply(queues, command, &reply);
        ...
        if (status == OK) {
            ...
            size_t bytesToRead =
                    std::min(remainingLength, static_cast<size_t>(reply.burstBytesReady));

            if (!queues.pcm->read(currentBufferPtr, bytesToRead)) {
