...
#include <vibratorflinger/HapticGeneratorConstants.h>
...
status_t HapticGeneratorSession::startStream(int32_t vibratorId,
    ...
    // If a stream already exists for this vibrator, close it before creating a new one
    closeStreamLocked(vibratorId);
...
status_t HapticGeneratorSession::readStream(int32_t vibratorId, size_t bufferSize, int8_t* buffer,
                                            size_t* bytesRead) {
    *bytesRead = 0;
    ...
    return streamIt->second->read(queueIt->second, bufferSize, buffer, bytesRead);
}
...
void HapticGeneratorSession::closeStreamLocked(int32_t vibratorId) {
    auto streamIt = mStreams.find(vibratorId);
    if (streamIt == mStreams.end()) {
...
status_t HapticGeneratorSession::close() {
    ...
    for (const auto& [vibratorId, queues] : mQueues) {
        ...
        sendCommandAndReceiveReplyLocked(vibratorId, command, &reply);
...
void HapticGeneratorSession::sendCommandAndReceiveReplyLocked(int32_t vibratorId,
