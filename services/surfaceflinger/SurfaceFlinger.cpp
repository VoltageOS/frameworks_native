...
gui::Uid getFilterUidFromContentAccessPolicy(
        gui::ISurfaceComposerDefault::ContentAccessPolicy contentAccessPolicy, gui::Uid ownerUid) {
    if (contentAccessPolicy == gui::ISurfaceComposerDefault::ContentAccessPolicy::SameUid) {
        return ownerUid;
    }
...
sp<IBinder> SurfaceFlinger::createVirtualDisplay(
        ...
        gui::ISurfaceComposerDefault::ContentAccessPolicy contentAccessPolicy,
    ...
    if (!isPrivileged) {
        ...
        if (contentAccessPolicy == gui::ISurfaceComposerDefault::ContentAccessPolicy::AllUids) {
            ALOGE("Only privileged processes can create a virtual display that includes content "
                  "from all UIDs.");
            return nullptr;
        }
        if (ownerUid != uid) {
            ALOGW("Only privileged processes can override the owner UID of a virtual display.");
            ownerUid = uid;
        }
    }
    ...
    state.contentAccessPolicy = contentAccessPolicy;
...
sp<DisplayDevice> SurfaceFlinger::setupNewDisplayDeviceInternal(
    ...
    display->setLayerFilter(makeLayerFilterForDisplay(display->getDisplayIdVariant(),
                                                     state.layerStack,
                                                     getFilterUidFromContentAccessPolicy(
                                                             state.contentAccessPolicy,
                                                             state.ownerUid)));
        ...
        if (currentState.layerStack != drawingState.layerStack) {
            display->setLayerFilter(
                    makeLayerFilterForDisplay(display->getDisplayIdVariant(),
                                              ...
                                              getFilterUidFromContentAccessPolicy(
                                                      currentState.contentAccessPolicy,
